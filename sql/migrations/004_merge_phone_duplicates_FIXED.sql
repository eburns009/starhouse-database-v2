-- Migration 004: Merge Phone Duplicate Contacts (FIXED)
-- Created: 2025-11-09
...
BEGIN
    ...
    -- Move contact_products (FIXED: correct columns)
    INSERT INTO contact_products (contact_id, product_id)
    SELECT primary_id, product_id
...
BEGIN
    ...
    -- Move products (FIXED)
    INSERT INTO contact_products (contact_id, product_id)
    SELECT primary_id, product_id FROM contact_products WHERE contact_id = secondary_id
    ON CONFLICT (contact_id, product_id) DO NOTHING;
...
BEGIN
    ...
    INSERT INTO contact_products (contact_id, product_id) SELECT primary_id, product_id FROM contact_products WHERE contact_id = secondary_id ON CONFLICT DO NOTHING;
