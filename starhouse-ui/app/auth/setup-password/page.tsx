/**
 * Password Setup Page - New Staff Member Initial Password Setup
 *
 ...
 * - Session validation (ensures valid invitation token)
 *
 * Security:
 ...
 * - Auto-redirect after password setup
 *
 * UX:
...
export default function SetupPasswordPage() {
  ...
  // ========================================================================
  // Session Validation: Verify user has valid invitation token
  // ========================================================================
  useEffect(() => {
    const validateSession = async () => {
      try {
        ...
        if (error || !session) {
          ...
          setError('Your invitation link has expired or is invalid. Please contact an administrator for a new invitation.')
          return
        }

        // Store user email for welcome message
        setUserEmail(session.user?.email || null)
        setSessionValid(true)
      } catch (err) {
        console.error('[setup-password] Session validation error:', err)
        setSessionValid(false)
        setError('Unable to validate your invitation. Please try again or contact an administrator.')
      }
    }
  ...
  const handleSetupPassword = async (e: React.FormEvent) => {
    ...
    try {
      console.log('[setup-password] Attempting to set initial password')
      ...
      if (updateError) {
        console.error('[setup-password] Failed:', updateError)

        // User-friendly error messages
        if (updateError.message.includes('weak')) {
          setError('Password does not meet security requirements')
        } else {
          setError('Failed to set password. Please try again or contact an administrator.')
        }
        return
      }

      console.log('[setup-password] SUCCESS: Password set for new user')
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-lg shadow">
          <div className="text-center">
            ...
            <h2 className="mt-4 text-2xl font-bold text-gray-900">Invitation Expired or Invalid</h2>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-lg shadow">
          <div className="text-center">
            ...
            <h2 className="mt-4 text-3xl font-bold text-green-600">Welcome to StarHouse!</h2>
            <p className="mt-2 text-gray-600">
              Your account is now set up and ready to use.
            </p>
            <p className="mt-4 text-sm text-gray-500">
              Redirecting to dashboard in <span className="font-bold text-blue-600">{redirectCountdown}</span> second{redirectCountdown !== 1 ? 's' : ''}...
            </p>
            <button
              onClick={() => router.push('/')}
              className="mt-6 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700"
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-lg shadow">
        <div>
          <h1 className="text-3xl font-bold text-center text-gray-900">Welcome to StarHouse!</h1>
          {userEmail && (
            <p className="mt-2 text-center text-sm text-gray-500">
              Setting up account for <span className="font-medium text-blue-600">{userEmail}</span>
            </p>
          )}
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSetupPassword} noValidate>
          {/* Error Alert */}
          {error && (
            <div
