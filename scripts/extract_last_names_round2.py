#!/usr/bin/env python3
"""
Extract Last Names from Email Patterns - Round 2
3 more customers with clear last name patterns in email addresses
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/extract_last_names_round2_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
EMAIL_PATTERN_EXTRACTIONS = [
    {
        'email': 'ryleetberry@gmail.com',
        'first_name': 'Rylee',  # Already have
        'last_name': 'Berry',  # Extract from ryleetberry (ryleet + berry)
        'revenue': 150.00,
        'pattern': 'ryleetberry → rylee + t + berry',
        'confidence': 'HIGH'
    },
    {
        'email': 'andywalunas@gmail.com',
        'first_name': 'Andrew',  # Already have (Andy)
        'last_name': 'Walunas',  # Extract from andywalunas (andy + walunas)
        'revenue': 133.00,
        'pattern': 'andywalunas → andy + walunas',
        'confidence': 'HIGH'
    },
    {
        'email': 'madelineheiar@gmail.com',
        'first_name': 'Madeline',  # Already have
        'last_name': 'Heiar',  # Extract from madelineheiar (madeline + heiar)
        'revenue': 37.00,
        'pattern': 'madelineheiar → madeline + heiar',
        'confidence': 'HIGH'
    }
]
...
def preview_extractions():
    ...
    logger.info("=" * 80)
    logger.info("PREVIEW: PROPOSED LAST NAME EXTRACTIONS - ROUND 2")
    logger.info("=" * 80)
    logger.info("")
...
def execute_extraction(conn, dry_run=False):
    ...
    if dry_run:
        ...
        logger.info("EXECUTING: Last Name Extraction - Round 2")
    logger.info("=" * 80)
    logger.info("")
...
def main():
    ...
    logger.info("")
    logger.info("╔" + "=" * 78 + "╗")
    logger.info("║" + "EXTRACT LAST NAMES FROM EMAIL PATTERNS - ROUND 2".center(78) + "║")
    logger.info("║" + "Quick Win: 3 customers, $320 revenue".center(78) + "║")
    logger.info("╚" + "=" * 78 + "╝")
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("✅ LAST NAME EXTRACTION ROUND 2 COMPLETE")
        logger.info("=" * 80)
        logger.info("")
        logger.info("Summary:")
        logger.info("  • 3 customers enriched with last names")
        logger.info("  • $320 revenue impact")
