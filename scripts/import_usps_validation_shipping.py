#!/usr/bin/env python3
"""
Import USPS validation data for SHIPPING addresses.

This script is SAFE because it:
1. Reads the original shipping address export (which has email addresses)
...
def read_original_shipping_list(filepath):
    """Read the original shipping address CSV (which has emails)."""
    contacts = []
...
def update_contact_usps_data(cursor, email, validation_data):
    """Update contact SHIPPING address with USPS validation data using email as the key."""
...
def main():
    ...
    # File paths
    original_file = '/tmp/shipping_addresses_for_validation.csv'
    ...
    if not os.path.exists(original_file):
        print(f"Error: Original shipping address list not found: {original_file}")
        sys.exit(1)
    ...
    if not os.path.exists(validation_file):
        ...
        print()
        print("Instructions:")
        print("1. Upload /tmp/shipping_addresses_for_validation.csv to USPS validation service")
        print("2. Download the validation results")
        print("3. Save results as /tmp/shipping_addresses_validated.csv")
        print("4. Run this script again")
        sys.exit(1)

    print("=" * 70)
    print("USPS VALIDATION IMPORT - SHIPPING ADDRESSES")
    print("=" * 70)
    print(f"Original shipping list: {original_file}")
    print(f"Validation output:      {validation_file}")
    print()

    # Read both files
    print("Reading original shipping address list...")
    original_contacts = read_original_shipping_list(original_file)
    ...
    print(f"Total contacts with USPS-validated shipping addresses: {result['count']}")

    # Show sample of validated addresses
    print("\n" + "=" * 70)
    print("SAMPLE VALIDATED SHIPPING ADDRESSES (first 5)")
    print("=" * 70)
    ...
    cursor.execute("""
        SELECT
            ...
            shipping_usps_precision,
            shipping_usps_dpv_match_code,
            shipping_usps_county
        FROM contacts
        WHERE shipping_usps_validated_at IS NOT NULL
        ORDER BY shipping_usps_validated_at DESC
        LIMIT 5
    """)

    for row in cursor.fetchall():
        print(f"\n{row['first_name']} {row['last_name']} ({row['email']})")
        print(f"  Original:   {row['shipping_address_line_1']}")
        print(f"  Validated:  {row['shipping_usps_delivery_line_1']}")
        print(f"  Last Line:  {row['shipping_usps_last_line']}")
        print(f"  Precision:  {row['shipping_usps_precision']} | Match: {row['shipping_usps_dpv_match_code']} | County: {row['shipping_usps_county']}")
