#!/usr/bin/env python3
"""
Extract Last Names from Email Patterns - Round 3
2 more customers with initial + last name patterns
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/extract_last_names_round3_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
EMAIL_PATTERN_EXTRACTIONS = [
    {
        'email': 'lbrada09@gmail.com',
        'first_name': 'Lauren',  # Already have
        'last_name': 'Brada',  # Extract from lbrada09 (l + brada)
        'revenue': 78.00,
        'pattern': 'lbrada09 → l + brada',
        'confidence': 'HIGH'
    },
    {
        'email': 'rkatz6@gmail.com',
        'first_name': 'Rachel',  # Already have
        'last_name': 'Katz',  # Extract from rkatz6 (r + katz)
        'revenue': 39.00,
        'pattern': 'rkatz6 → r + katz',
        'confidence': 'HIGH'
    }
]
...
def preview_extractions():
    ...
    logger.info("=" * 80)
    logger.info("PREVIEW: PROPOSED LAST NAME EXTRACTIONS - ROUND 3")
    logger.info("=" * 80)
    logger.info("")
...
def execute_extraction(conn, dry_run=False):
    ...
    if dry_run:
        ...
        logger.info("EXECUTING: Last Name Extraction - Round 3")
    logger.info("=" * 80)
    logger.info("")
...
def main():
    ...
    logger.info("")
    logger.info("╔" + "=" * 78 + "╗")
    logger.info("║" + "EXTRACT LAST NAMES FROM EMAIL PATTERNS - ROUND 3".center(78) + "║")
    logger.info("║" + "Quick Win: 2 customers, $117 revenue".center(78) + "║")
    logger.info("╚" + "=" * 78 + "╝")
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("✅ LAST NAME EXTRACTION ROUND 3 COMPLETE")
        logger.info("=" * 80)
        logger.info("")
        logger.info("Summary:")
        logger.info("  • 2 customers enriched with last names")
        logger.info("  • $117 revenue impact")
        logger.info("  • Extracted from email patterns (initial + last name)")
