#!/usr/bin/env python3
"""
PayPal Transaction Import Script - FIXED VERSION (FAANG Quality)
=================================================================
Imports PayPal transaction data and enriches contact records.

CRITICAL FIX:
-------------
This version correctly handles subscriptions by:
1. NEVER storing PayPal IDs in kajabi_subscription_id field
2. Only storing PayPal IDs in paypal_subscription_reference field
3. Checking for existing subscriptions before creating new ones
4. Matching by contact + amount + billing cycle to avoid duplicates

ROOT CAUSE OF BUG IN ORIGINAL:
-------------------------------
Line 465 of original script:
    INSERT INTO subscriptions (..., kajabi_subscription_id, ...) VALUES (..., subscription_ref, ...)

This stored PayPal subscription references (I-XXXXXX) in the kajabi_subscription_id field,
causing duplicates when Kajabi subscriptions already existed.

CORRECT BEHAVIOR:
-----------------
- kajabi_subscription_id: ONLY for Kajabi subscription IDs (numeric, from Kajabi imports)
- paypal_subscription_reference: For PayPal subscription IDs (I-XXXXXX format)
- Before creating subscription, check if one already exists for this contact with same amount/cycle
...
class PayPalImporter:
    def __init__(self, connection_string: str, dry_run: bool = True, corrections_file: Optional[str] = None):
        ...
        self.stats = {
            ...
            'subscriptions_skipped_duplicate': 0,
    ...
    def parse_name(self, name_str: str) -> tuple:
    ...
    def find_contact_by_name(self, first_name: str, last_name: str) -> Optional[Dict]:
    ...
    def find_existing_subscription(self, contact_id: str, amount: Decimal, billing_cycle: str) -> Optional[Dict]:
        ...
        # Check for existing subscription with same contact + similar amount + same cycle
        self.cursor.execute("""
            SELECT
                *,
                CASE
                    WHEN lower(billing_cycle) IN ('month', 'monthly') THEN 'month'
                    WHEN lower(billing_cycle) IN ('year', 'annual', 'yearly') THEN 'annual'
                    ELSE lower(billing_cycle)
                END as normalized_billing_cycle
            FROM subscriptions
            WHERE contact_id = %s
              AND deleted_at IS NULL
              AND status IN ('active', 'canceled')
              AND ABS(amount - %s) <= 1.0
            HAVING normalized_billing_cycle = %s
            LIMIT 1
    ...
    def create_or_update_subscription(self, subscription_ref: str, transactions: List[Dict]):
        """
        Create or update subscription from grouped transactions.

        CRITICAL FIX: This version correctly:
        1. Sets kajabi_subscription_id to NULL (not the PayPal reference!)
        2. Only sets paypal_subscription_reference to the PayPal ID
        3. Checks for existing subscriptions before creating duplicates
        """
        if not transactions:
            return
        ...
        avg_amount = Decimal(str(total_paid / len(transactions)))
        ...
        if not self.dry_run:
            # CRITICAL FIX: Check if subscription already exists for this contact + amount + cycle
            existing_sub = self.find_existing_subscription(contact_id, avg_amount, billing_cycle)

            if existing_sub:
                # Subscription already exists - just update the PayPal reference if needed
                if not existing_sub.get('paypal_subscription_reference'):
                    self.cursor.execute("""
                        UPDATE subscriptions SET
                            ...
                            updated_at = now()
                        WHERE id = %s
                    """, (subscription_ref, existing_sub['id']))
                    self.stats['subscriptions_updated'] += 1
                else:
                    # Already has PayPal reference - skip
                    self.stats['subscriptions_skipped_duplicate'] += 1
            else:
                # Create new subscription (no existing match found)
                start_date = datetime.strptime(f"{first_txn['Date']} {first_txn['Time']}", "%m/%d/%Y %H:%M:%S")

                # CRITICAL FIX: kajabi_subscription_id is set to NULL (not the PayPal reference!)
                self.cursor.execute("""
                ...
                """, (
                    contact_id,
                    None,  # CRITICAL FIX: NULL, not subscription_ref!
                    subscription_ref,  # PayPal reference goes here
                    membership_product_id,
                    status,
                    avg_amount,
    ...
    def print_stats(self):
        ...
        print(f"  - Skipped (duplicate): {self.stats['subscriptions_skipped_duplicate']}")
...
def main():
    parser = argparse.ArgumentParser(description='Import PayPal transactions (FIXED VERSION)')
