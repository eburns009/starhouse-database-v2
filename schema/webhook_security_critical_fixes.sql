...
RAISE NOTICE '✅ RLS enabled on webhook_events - only service_role can access';
...
-- Performance: Only index recent nonces (last 15 minutes)
-- Old nonces auto-expire and can be cleaned up
...
COMMENT ON FUNCTION process_webhook_atomically IS
  'Atomically check idempotency and record webhook event. Prevents race conditions.';
